...
class AudioLoop:
    ...
    def _get_frame(self, cap, detect_motion=False):
        ...
        ret, frame = cap.read()
        if not ret:
            return (None, False, None) if detect_motion else (None, None)
        ...
        with io.BytesIO() as image_io:
            ...
            return (blob, motion_detected, display_frame) if detect_motion else (blob, display_frame)
    ...
    async def get_frames(self):
        ...
        while self.running:
            if time.time() - self.last_video_send >= self.video_send_interval:
                ...
                if frame_result:
                    blob, motion_detected, display_frame = frame_result
                    if blob:
